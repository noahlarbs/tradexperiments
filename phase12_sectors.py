...
os.makedirs("data/phase12_results", exist_ok=True)

TICKERS = ["XLF", "XLV", "XLK", "XLE", "XLI", "XLP", "XLY", "XLU", "XLB"]

print(f"\n[1/5] Downloading {len(TICKERS)} Sector ETFs from {START} to {END}...")
...
prices.to_csv("data/phase12_results/prices.csv")
log_returns.to_csv("data/phase12_results/log_returns.csv")
...
returns_df.to_csv("data/phase12_results/daily_returns.csv")
...
def display_metrics(df_returns, title):
    hrp_sr = ann_sharpe(df_returns["HRP"])
    rows = []
    for name in df_returns.columns:
        r, v, sh, dd = metrics(df_returns[name])
        psr_val = psr(df_returns[name], hrp_sr) if name != "HRP" else np.nan
        rows.append({
...
met_full = display_metrics(returns_df, "Phase 12 Metrics (Full History 2000-2026)")
met_full.to_csv("data/phase12_results/metrics_full.csv")

# Post-2010 Subset
returns_df_2010 = returns_df[returns_df.index >= '2010-01-01']
met_2010 = display_metrics(returns_df_2010, "Phase 12 Metrics (2010-2026 Sub-Period)")
met_2010.to_csv("data/phase12_results/metrics_2010.csv")
...
ax.set_title(f"Cumulative Returns — Intra-Asset Sectors ({n} Assets)", fontsize=13)
ax.set_ylabel("Growth of $1")
ax.legend(fontsize=8.5, ncol=2, loc="upper left")
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("data/phase12_results/cumulative_returns.png", dpi=150)
plt.close()
